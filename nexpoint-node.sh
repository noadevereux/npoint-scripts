...
install_Nexpoint_node() {
    # Fetch releases
    mkdir -p "$DATA_DIR"
    mkdir -p "$APP_DIR"
    mkdir -p "$DATA_MAIN_DIR"
    
    # ÐŸÑ€Ð¾Ð²ÐµÑ€ÐºÐ° Ð½Ð° ÑÑƒÑ‰ÐµÑÑ‚Ð²Ð¾Ð²Ð°Ð½Ð¸Ðµ Ñ„Ð°Ð¹Ð»Ð° Ð¿ÐµÑ€ÐµÐ´ ÐµÐ³Ð¾ Ð¾Ñ‡Ð¸ÑÑ‚ÐºÐ¾Ð¹
    if [ -f "$CERT_FILE" ]; then
        >"$CERT_FILE"
    fi
...
services:
    ...
    network_mode: host
    environment:
    SSL_CLIENT_CERT_FILE: "/var/lib/nexpoint-node/cert.pem"
...
EOL
    ...
    cat >> "$COMPOSE_FILE" <<EOL
        ...
        volumes:
            - $DATA_MAIN_DIR:/var/lib/nexpoint
            - $DATA_DIR:/var/lib/nexpoint-node
EOL
...
show_Nexpoint_node_logs() {
    $COMPOSE -f $COMPOSE_FILE -p "$APP_NAME" logs
}

follow_Nexpoint_node_logs() {
...
install_command() {
    ...
    up_Nexpoint_node
    follow_Nexpoint_node_logs
    echo "Use your IP: $NODE_IP and defaults ports: $SERVICE_PORT and $XRAY_API_PORT to setup your Nexpoint Main Panel"
}
