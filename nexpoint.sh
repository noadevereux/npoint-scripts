...
DATA_DIR="/var/lib/${APP_NAME_LOWER}"
...
send_backup_to_telegram() {
    ...
    local split_dir="/tmp/Nexpoint_backup_split"
...
backup_command() {
    ...
    local temp_dir="/tmp/Nexpoint_backup"
    local timestamp=$(date +"%Y%m%d%H%M%S")
    local backup_file="$backup_dir/backup_$timestamp.tar.gz"
    local error_messages=()
    local log_file="/var/log/Nexpoint_backup_error.log"
    > "$log_file"
    ...
    rsync -av --exclude 'xray-core' --exclude 'mysql' "$DATA_DIR/" "$temp_dir/Nexpoint_data/" >>"$log_file" 2>&1
...
install_Nexpoint() {
    ...
    services:
        Nexpoint:
            ...
            network_mode: host
            volumes:
                - /var/lib/nexpoint:/var/lib/nexpoint
                - /var/lib/nexpoint/logs:/var/lib/nexpoint-node
            depends_on:
                mariadb:
...
EOF
    ...
    sed -i 's~^\(SQLALCHEMY_DATABASE_URL = "sqlite:////var/lib/[Nn]expoint/db.sqlite3"\)~#\1~' "$APP_DIR/.env"
    sed -i 's~^\(SQLALCHEMY_DATABASE_URL = "sqlite:////var/lib/[Nn]expoint/db.sqlite3"\)~#\1~' "$APP_DIR/.env"
...
EOF
    ...
    sed -i 's~^\(SQLALCHEMY_DATABASE_URL = "sqlite:////var/lib/[Nn]expoint/db.sqlite3"\)~#\1~' "$APP_DIR/.env"
        ...
        sed -i 's~\(XRAY_JSON = \).*~\1"/var/lib/nexpoint/xray_config.json"~' "$APP_DIR/.env"
        sed -i 's~\(SQLALCHEMY_DATABASE_URL = \).*~\1"sqlite:////var/lib/nexpoint/db.sqlite3"~' "$APP_DIR/.env"
...
prompt_for_Nexpoint_password() {
    ...
    # ÐŸÐ°ÑƒÐ·Ð° 3 ÑÐµÐºÑƒÐ½Ð´Ñ‹ Ð¿ÐµÑ€ÐµÐ´ Ð¿Ñ€Ð¾Ð´Ð¾Ð»Ð¶ÐµÐ½Ð¸ÐµÐ¼
    sleep 3
}
...
install_command() {
    ...
    Nexpoint_version="latest"
    Nexpoint_version_set="false"
    ...
    while [[ $# -gt 0 ]]; do
        ...
        case $key in
            ...
            ;;
            --dev)
                if [[ "$Nexpoint_version_set" == "true" ]]; then
                    colorized_echo red "Error: Cannot use --dev and --version options simultaneously."
                    exit 1
                fi
                Nexpoint_version="dev"
                Nexpoint_version_set="true"
                shift
            ;;
            --version)
                if [[ "$Nexpoint_version_set" == "true" ]]; then
    ...
    check_version_exists() {
        local version=$1
    repo_url="https://api.github.com/repos/noadevereux/npoint-vpn/releases"
    ...
    if [[ "$Nexpoint_version" == "latest" || "$Nexpoint_version" == "dev" || "$Nexpoint_version" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        if check_version_exists "$Nexpoint_version"; then
            install_Nexpoint "$Nexpoint_version" "$database_type"
            echo "Installing $Nexpoint_version version"
        else
            echo "Version $Nexpoint_version does not exist. Please enter a valid version (e.g. v0.5.2)"
            exit 1
        fi
    else
